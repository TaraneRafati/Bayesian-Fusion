...
SAVE_PLOT_PATH = "fig_normalized_slice_violin_overlay.pdf"
OVERLAY_MODE = True
...
def plot_overlaid_violin(plot_df, n_decrease, n_increase, n_unchanged, save_plot_path):
    if plot_df.empty:
        print("[WARN] No non-zero differences found — nothing to plot.")
        return

    colors = DIRECTION_PALETTE
    x_center = 0
    width = 0.8
    ...
    for direction, color in colors.items():
        vals = plot_df.loc[plot_df["direction"] == direction, "normalized_slice"].values
        if len(vals) < 2:
            print(f"[WARN] Fewer than 2 points for '{direction}' — skipping its violin.")
            continue
        ...
        ax.fill_betweenx(y_grid, x_center, x_center + density,
                          color=color, alpha=0.45, label=direction, linewidth=1.2,
                          edgecolor=color)

        median = np.median(vals)
        ax.hlines(median, x_center, x_center + width / 2,
                  color=color, linestyle='--', linewidth=1.5, alpha=0.9)

    ax.set_xlim(-0.15, width / 2 + 0.15)
    ax.set_xticks([])
    ax.set_ylabel("Normalized slice position (0 = first, 1 = last)")
    ax.set_ylim(-0.05, 1.05)
    ax.set_title(
        "Normalized Slice Position: Decrease vs. Increase (overlaid)\n"
        f"(n_decrease={n_decrease}, n_increase={n_increase}, n_unchanged={n_unchanged})"
    )
...
def run(csv_path, diff_column=DIFF_COLUMN, patient_col=PATIENT_COLUMN,
        slice_col=SLICE_COLUMN, save_plot_path=SAVE_PLOT_PATH, overlay_mode=OVERLAY_MODE):
    ...
    if overlay_mode:
        plot_overlaid_violin(plot_df, n_decrease, n_increase, n_unchanged, save_plot_path)
    else:
